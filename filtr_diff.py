...
def block_filtr_diff(spectr_tmp, spectr_fltr, spectr_diff, file_name_tmp_b): ## function of spectrum treatment
...
## Creating list of peaks
	...
	for i in range (0, numb-1, 1):
		...
		data_line_diff = spectr_diff.get_spectrum(i)
		...
		data_line_diff.set_val(0, 0)
		data_line_diff.set_val(nx_res-1, 0)
		for k in range(1, nx_res-2, 1):
			diff0 = (filtr_tmp[k+1] - filtr_tmp[k-1])/(voltage[k+1] - voltage[k-1])
			data_line_diff.set_val(k, diff0)
		spectr_diff.set_spectrum(i, data_line_diff)
	return spectr_fltr, spectr_diff
...
for i in range(n_list):
	...
	try: ## exception exicting topography data .tf0
		...
		container3 = gwy.gwy_file_load(file_list[i], gwy.RUN_NONINTERACTIVE)
	except glib.GError:
		print('File  %s have not topography data' %file_name)
		with open('/home/simora/STM_STS/OMICRON_DATA/%s_topo_error.dat' % file_name, 'w') as file:
		...
		gwy.gwy_app_data_browser_add(container3)
		try: ## exception exicting spectroscopy data .sf0
			...
			with open('/home/simora/STM_STS/OMICRON_DATA/%s_spectro_error.dat' % file_name, 'w') as file:
			...
			gwy.gwy_process_func_run('omicron', container3, gwy.RUN_IMMEDIATE)
			gwy.gwy_process_func_run('spectro', container3, gwy.RUN_IMMEDIATE)
			spectr1 = container1['/sps/0']## spectroscopy is data field fron gwy.Container '/sps/0', here we have ONE spectra set!!!
			spectr2 = container2['/sps/0']
			spectr3 = container3['/sps/0']
			block_filtr_diff(spectr1, spectr2, spectr3, file_name) ## function of spectrum treatment 
			gwy_app_file_write(container2, '/home/simora/STM_STS/FILTR/%s_filtr.gwy' %file_name)
			gwy_app_file_write(container3, '/home/simora/STM_STS/DERIVATIVE/%s_diff.gwy' %file_name)
			gwy.gwy_app_data_browser_remove(container1)
			gwy.gwy_app_data_browser_remove(container2)	
			gwy.gwy_app_data_browser_remove(container3)
print('／ﾌﾌ 　　　　　 　　 　ム｀ヽ\n' + '/ ノ)　　 ∧　　∧　　　　）　ヽ\n' + '/ ｜　　(´・ω ・`）ノ⌒（ゝ._,ノ\n' + '/　ﾉ⌒＿⌒ゝーく　 ＼　　／\n' + '丶＿ ノ 　　 ノ､　　|　/\n' + '　　 `ヽ `ー-‘人`ーﾉ /\n' + '　　　 丶 ￣ _人’彡ﾉ\n' + '　　　／｀ヽ _/\__')
